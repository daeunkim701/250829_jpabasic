...
public class MemberController {
    private final MemberService memberService; // 직접 생성자를 만들어도 되지만.
    ...
    @PostMapping("/new") // GET /new -> FORM -> POST -> Redirect -> GE
    public String create(@ModelAttribute Member member) {
        memberService.saveMember(member);
        return "redirect:/members"; // 하나가 추가된 다음에 -> GET -> 모든 데이터를 보여주는 방식으로 됨
    }
}
